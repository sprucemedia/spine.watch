  ...
  Watch = {
    prepareWatch: function(model, callback) {
      ...
      bind = function(record, prop, handler) {
        var getter, setter;
        if ((record[prop] != null) && record.hasOwnProperty(prop)) {
        ...
        setter = function(value) {
          ...
          if ((!Spine.isArray(record["_" + prop])) && record["_" + prop] && value && typeof record["_" + prop] === 'object' && typeof value === 'object' && Object.getPrototypeOf(record["_" + prop]) === Object.getPrototypeOf(value)) {
            return;
          }
          previous = record["_" + prop];
          record["_" + prop] = value;
          if (window.DebugLevel >= 8) {
            console.log("Spine.Watch: Updating " + prop + " from '" + previous + "' to '" + record["_" + prop] + "'");
          }
      ...
      trigger = function(prop, previous, current) {
        if (callback) {
          ...
          this.trigger("update[]", current, prop, previous);
